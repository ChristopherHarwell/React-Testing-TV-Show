...
import {fetchShow as fetches} from "../src/api/fetchShow";
...
fetches.mockResolvedValueOnce(showData);
test("Successfully renders data from the API.", async () => {
  const {findByTestId, findByText, findAllByTestId } = render(<App />);
  const fetchText = await findByText(/fetching data/i);

  const getButton = findByText(/Select a season/i)
  userEvent.click(findByTestId(/dropdown/i));
  const getEpisode =  findAllByTestId(/episode/i)
